	...
	return (
		...
		>
			<>
				...
				<div
					className={`advancement advancement-${getEraName(adv.era).toLowerCase()}`}
					onClick={(_) => {
						adv.disabled
							? noop()
				...
				>
					...
					>
						...
						{adv.purchased && (
							<div style={{ fontSize: '12px' }}>Discovered in {adv.purchasedDate} AC</div>
						)}
					</div>
					<div
...
> {
	...
	public render() {
		...
		return (
			<Screen type="Advancements" color={colors.get('advancements')} style={{ overflowY: 'auto' }}>
				<div
					style={{
						display: 'flex',
