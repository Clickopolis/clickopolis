      ...
      <>
        ...
        {resource.foodBonus ? (
          <div>
            ...
            {resource.foodBonus} per {resource.name},{' '}
            {(resource.foodBonus * resource.total).toFixed(0)} total
          </div>
        ) : null}
        {resource.healthBonus ? (
          <div>
            <img style={{ height: '1rem' }} src="./images/health.svg" />+
            {resource.healthBonus} per {resource.name},{' '}
            {(resource.healthBonus * resource.total).toFixed(0)} total
          </div>
        ) : null}
        {resource.faithBonus ? (
          <div>
            <img style={{ height: '1rem' }} src="./images/faith.svg" />+
            {resource.faithBonus} per {resource.name},{' '}
            {(resource.faithBonus * resource.total).toFixed(0)} total
          </div>
        ) : null}
        {resource.cultureBonus ? (
          <div>
            <img style={{ height: '1rem' }} src="./images/culture.svg" />+
            {resource.cultureBonus} per {resource.name},{' '}
            {(resource.cultureBonus * resource.total).toFixed(0)} total
          </div>
        ) : null}
      </>
    );
    ...
    return (
      <div className={css.subresourcesScreen}>
        <h3 className={css.subresourcesScreenTitle}>
          <img
            className={css.subresourcesScreenIcon}
            src="./images/subresources.svg"
          />
          Subresources
        </h3>

        <div className="resources-matrix-row">
          {this.renderResourceRow(resources)}
        </div>
