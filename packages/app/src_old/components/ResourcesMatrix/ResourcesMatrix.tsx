...
export class ResourcesMatrixBase extends React.Component<
	ResourcesMatrixProps,
	{ info: Resource | boolean }
> {
	...
	renderUnlocked = (resource: Resource) => {
		return (
			<>
				<div style={{ marginBottom: '1rem' }}>{resource.total} owned</div>
				<div style={{ marginBottom: '0.5rem' }}>{resource.description}</div>
				{resource.foodBonus ? (
					<div>
						<img style={{ height: '1rem' }} src="./images/food.svg" />+{resource.foodBonus} per{' '}
						{resource.name}, {(resource.foodBonus * resource.total).toFixed(0)} total
					</div>
				) : null}
				{resource.healthBonus ? (
					<div>
						<img style={{ height: '1rem' }} src="./images/health.svg" />+{resource.healthBonus} per{' '}
						{resource.name}, {(resource.healthBonus * resource.total).toFixed(0)} total
					</div>
				) : null}
				{resource.faithBonus ? (
					<div>
						<img style={{ height: '1rem' }} src="./images/faith.svg" />+{resource.faithBonus} per{' '}
						{resource.name}, {(resource.faithBonus * resource.total).toFixed(0)} total
					</div>
				) : null}
				{resource.cultureBonus ? (
					<div>
						<img style={{ height: '1rem' }} src="./images/culture.svg" />+{resource.cultureBonus}{' '}
						per {resource.name}, {(resource.cultureBonus * resource.total).toFixed(0)} total
					</div>
				) : null}
			</>
		);
	...
	renderLocked = (resource: Resource) => {
		return <div>{addImages((resource as any).unlockRequirements)}</div>;
	};
	...
	public render() {
		...
		return (
			<div className={css.subresourcesScreen}>
				...
				<div className="resources-matrix-row">{this.renderResourceRow(resources)}</div>
