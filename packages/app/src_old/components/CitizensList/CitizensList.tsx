...
export class CitizensListBase extends React.PureComponent<CitizensListProps> {
	...
	private calculateAllContributions() {
		...
		const findContribs = (type: string, resource: string) =>
			citizens
				.map((citizen) => {
					return (
						citizen.amount *
						(citizen?.contribution?.find(
							(contrib) => contrib.type === type && contrib.resource === resource
						)?.amount || 0)
					);
				})
	...
	private addCitizen(amount: number, c: Citizen) {
		...
		const fullEmployment =
			this.props.civilization.population -
				this.props.citizens
					.map((c) => c.amount)
					.reduce((prev, curr) => {
						return prev + curr;
		...
		return citizens.map((c: Citizen) => {
			return (
				<div
					title={c.name}
					style={{
						background: this.getCitizenColor(c.name),
						height: '1rem',
						width: `${(c.amount / population) * 100}%`
					}}
				>
					<Tooltip title={`${((c.amount / population) * 100).toFixed(1)}% ${c.name}s`}>
		...
		return this.props.citizens.map((c: Citizen, idx: number) => {
			return (
				<div className="citizens-list-item" key={idx}>
					...
					<div
						className="citizen-contribution"
						style={{ display: 'flex', justifyContent: 'center' }}
					>
						{Array.isArray(c.contribution)
							? c.contribution.map((contrib: Contribution, index: number) => (
	...
	public render() {
		...
		return (
			<div className="citizens-list-wrapper">
				...
				<div className="citizens-list">{this.renderCitizens()}</div>
				<div className="total-citizen-contributions" style={{ color: 'white' }}>
					<span>Total Citizen Contributions</span>

					<div>
						{abbrNum(contributions.foodPerClick)}{' '}
						<img style={{ height: '1rem' }} src="./images/food.svg" /> PC
					</div>

					<div>
						{abbrNum(contributions.foodPerSecond)}{' '}
						<img style={{ height: '1rem' }} src="./images/food.svg" /> PS
					</div>

					<div>
						{abbrNum(contributions.productionPerSecond)}{' '}
						<img style={{ height: '1rem' }} src="./images/production.svg" /> PS
					</div>
				</div>
			</div>
		);
...
export const CitizensList: any = connect(
	(state: any) => ({
		citizens: [state.ruler, state.farmer, state.miner, state.soldier, state.merchant],
