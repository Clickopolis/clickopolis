...
export class EventsBase extends React.Component<EventsProps> {
	...
	rollEvent() {
		const { cattle, civilization, addResource, addNotification, farmer, miner } = this.props;
		...
		addNotification({
			content: (
				<div>
					One of your farmers has domesticated a cow. Say hi to{' '}
					{choose(['Bessy', 'Betty', 'Martha', 'Jumbo', 'Mac', 'Beefy Dan'])}!
					<br />
		...
		addNotification({
			content: (
				<div>
					...
					<br />+{farmer.amount} <img src="./images/maize.svg" />
				</div>
			),
			id: uuid()
		});
		...
		addNotification({
			content: (
				<div>
					...
					<br />+{miner.amount} <img src="./images/stone.svg" />
				</div>
			),
			id: uuid()
		});
