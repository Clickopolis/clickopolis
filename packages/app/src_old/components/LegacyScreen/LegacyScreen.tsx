  ...
  return (
    <div className={css.legacyItemWrapper}>
      ...
      <div className={css.legacyItem}>
        <div className={css.legacyName}>{leg.name.replace(/-/g, ' ')}</div>

        <div className={css.legacyLevel}>{leg.level}</div>
      </div>
    </div>
  );
};
...
export class LegacyScreenBase extends React.Component<LegacyScreenProps> {
  ...
  public render() {
    ...
    return (
      <Screen type="Legacy" color={colors.get('legacy')}>
        <div
          style={{
            margin: '1rem',
            display: 'flex',
        ...
        >
          ...
          >
            <img
              style={{ marginLeft: '.5rem', height: '1.5rem' }}
