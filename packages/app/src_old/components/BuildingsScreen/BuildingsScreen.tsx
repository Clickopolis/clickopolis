...
export const BuildingsDisplay = (
	build: Building & { disabled: boolean } & {
		purchaseBuilding: typeof purchaseBuilding;
	}
) => {
	return (
		...
		>
			{build.requirements.length ? (
				<div
					className="advancement-requirements"
					style={{
						position: 'absolute',
						top: '2rem',
						left: '25%',
			...
			>
				<img
					...
					src={`./images/buildings/${build.name.toLowerCase().replace(/\s/, '-')}.svg`}
				/>
				<Indicator
			...
			>
				<div style={{ textAlign: 'center' }} className="advancement-name">
					{build.name}
				</div>
				{
					<ul style={{ fontSize: '12px', display: 'flex', listStyleType: 'none' }}>
						{build.results.map((res) => (
							...
							>
								<img
									style={{ height: '.75rem', margin: '0 .25rem' }}
...
export class BuildingsScreenBase extends React.Component<BuildingScreenProps> {
	...
	public render() {
		...
		return (
			<Screen type="Buildings" color={colors.get('buildings')} style={{ overflowY: 'auto' }}>
				...
				<Subscreen name="Wonders" includeMarginTop>
					{''}
				</Subscreen>
			</Screen>
		);
