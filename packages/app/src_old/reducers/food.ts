...
export function food(
	state = foodState,
	action: Action<
		GROW_FOOD | CONSUME_FOOD | UPDATE_FOOD_PER_SECOND | UPDATE_FOOD_PER_CLICK | UPDATE_MAX_FOOD
	>
) {
	switch (action.type) {
		case GROW_FOOD:
			return {
				...state,
				total: Math.max(
					state.total + action.amount > state.max ? state.max : state.total + action.amount,
					0
				)
			};
		case CONSUME_FOOD:
