...
const handleCases = (
  state: Resource,
  action: Action<ADD_RESOURCE | UNLOCK_RESOURCE>,
  name: string
) => {
  switch (action.type) {
    case ADD_RESOURCE:
...
export function cattle(
  state = defaults.get('cattle'),
  action: Action<ADD_RESOURCE>
) {
  return handleCases(state, action, 'cattle');
}

export function cactus(
  state = defaults.get('cactus'),
  action: Action<ADD_RESOURCE>
) {
...
export function spaghetti(
  state = defaults.get('spaghetti'),
  action: Action<any>
) {
  switch (action.type) {
    case '_':
...
export function chihuahuas(
  state = defaults.get('chihuahuas'),
  action: Action<any>
) {
  switch (action.type) {
    case '_':
