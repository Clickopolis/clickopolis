...
export function StartScreenOptions(props: StartScreenOptionsProps) {
    ...
    return (
        <div className={css.startScreenOptions} style={{
            display: 'flex',
            paddingTop: '2rem',
            ...
            <div className={css.leadersWrapper}>
                ...
                <div className={css.selectedLeader}>
                    <div className={css.selectedLeaderCiv}>{selectedLeader?.defaultCivName}</div>
                        {selectedLeader && <figure className={classes(css.leaderProfile, css.leaderProfileInSelected)}>
                            <img style={{width: '100%'}} src={`./images/${selectedLeader?.name?.toLowerCase().replace(/\s/g, '-')}.png`} />
                        </figure>}
                    <div className={css.selectedLeaderName}>{selectedLeader?.name}</div>
                    
                    {selectedLeader?.abilities?.map((ab: Leader['abilities'][number]) => {
                        return <div style={{margin: '.5rem', marginTop: '1rem'}}>
                            <strong style={{display: 'block', fontSize: '110%', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '0.5rem',}}>{ab.name} {!ab?.unlocked && <span className={css.leaderLocked}>locked</span>}</strong>
                            <p style={{margin: '0'}}>{addImages(ab.description
                            )}</p>
                        </div>
                    })}
                </div>
