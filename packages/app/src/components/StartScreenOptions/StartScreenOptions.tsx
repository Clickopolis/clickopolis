...
export function StartScreenOptions() {
  const selectedLeader = useSelector<State, State['leader']>(
    (state) => state.leader
  );
  const dispatch = useDispatch();
    ...
    >
      ...
      <div className={css.leadersWrapper}>
        ...
        <div className={css.selectedLeader}>
          <div className={css.selectedLeaderCiv}>
            {selectedLeader?.defaultCivName}
          </div>
          {selectedLeader && (
            <figure
              className={classes(
                css.leaderProfile,
                css.leaderProfileInSelected
              )}
            >
              <img
                style={{ width: '100%' }}
                src={`./images/${selectedLeader?.name
                  ?.toLowerCase()
                  .replace(/\s/g, '-')}.png`}
              />
            </figure>
            ...
            return (
              <div style={{ margin: '.5rem', marginTop: '1rem' }}>
                ...
                >
                  {ab.name}{' '}
                  {!ab?.unlocked && (
                    <span className={css.leaderLocked}>locked</span>
                  )}
                </strong>
      ...
      <Button
        onClick={() => dispatch(turnOnFlag('HAS_STARTED_GAME'))}
