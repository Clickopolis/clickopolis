...
export function Subscreen ({children, name, icon, includeMarginTop}: SubscreenProps) {
    return <div className={classes(css.subscreen, includeMarginTop && css.subscreenMarginTop)}>
    
        <h3 className={classes(css.subscreenTitle)}>
            <img className={classes(css.subscreenIcon)} src={`./images/${name.toLowerCase().replace(/\s/, '-') || icon}.svg`} />
            {name}
        </h3>
