...
export const AdvancementDisplay = (
  adv: Advancement & {
    ac: number;
    purchaseAdvancement: typeof purchaseAdvancement;
    disabled: boolean;
  ...
  const areParentsPurchased = adv?.parents
    ? adv?.parents?.every((parent) => {
  ...
  useEffect(() => {
    const isParentPurchased = adv.advs.find(
      (ad) => ad.name === adv.parents?.[0]
    )?.purchased;
    ...
    const stroke = areBothPurchased
      ? [
          ...
          { color: colors.get('advancementPurchased'), pos: 1 },
        ]
      ...
      : [
          { color: colors.get('advancements'), pos: 0 },
          { color: colors.get('advancements'), pos: 1 },
        ];
    ...
    document
      .querySelectorAll('.path-segment')
      .forEach((p) => p.classList.add(css.advancementPath));
  ...
  return (
    ...
    >
      ...
      {adv.tier !== 0 && adv.arrow && (
        ...
        >
          <path
            className={css.advancementPath}
            d="M 0 0 C 0 0, 0 0, 0 160"
            stroke={colors.get('advancements')}
            strokeWidth="8px"
            fill="transparent"
            ref={svgPath}
          />
      ...
      <div
        className={`advancement advancement-${getEraName(
          adv.era
        ).toLowerCase()}`}
        onClick={(_) => {
          adv.disabled
            ? noop()
            : adv.purchaseAdvancement(
                omit(['purchaseAdvancement', 'ac', 'disabled'], adv),
                adv.ac
              );
        }}
        style={{
          backgroundColor: adv.purchased
            ? colors.get('advancementPurchased')
            : colors.get('advancements'),
      ...
      >
        <div
          className="advancement-info"
          style={{
            textAlign: 'center',
          }}
        >
          <div style={{}} className="advancement-name">
            {adv.name}
          </div>
          {/* <div>Tier {adv.tier} / {adv.subtier}</div> */}
          {adv.purchased && (
            <div style={{ fontSize: '12px' }}>
              Discovered in {adv.purchasedDate} AC
            </div>
          )}
        </div>
        <div
          className="advancement-icon"
          style={{
            display: 'flex',
            justifyContent: 'center',
            alignItems: 'center',
        ...
        >
          <img
            ...
            src={`./images/advancements/${
              adv.unlocked
                ? adv.name.toLowerCase().replace(/\s/, '-')
                : 'locked'
            }.svg`}
          />
        </div>
        <div
          className="advancement-categories"
          style={{
            display: 'flex',
            flexDirection: 'column',
            justifyContent: 'center',
        ...
        {!adv.purchased && adv.unlocked && (
          <div
            className="advancement-cost"
            style={{
              background: 'white',
              borderRadius: '.25rem',
...
export class AdvancementScreenBase extends React.Component<
  AdvancementScreenProps,
  { showDiscovered: boolean; x: number; y: number }
> {
  public state = {
  ...
  public render() {
    ...
    return (
      ...
      >
        ...
        >
          {this.props.advancements.map((adv, idx, advs) => (
            <AdvancementDisplay
              idx={idx}
              advs={advs}
              disabled={adv.cost > civilization.research.total}
              purchaseAdvancement={purchaseAdvancement}
              ac={civilization.ac}
              key={adv.name}
              {...adv}
            />
