...
const css = stylesheet({
    ...
    biomeRank: {
        ...
        maxWidth: '14rem',
        borderRadius: '.25rem',
        borderBottom: `4px solid ${color(colors.get('biomes')).darken(0.1).toString()}`,
        textAlign: 'center',
        margin: '.25rem auto',
...
export function BiomesScreen(props: BiomesScreenProps) {
    ...
    return <div className={classes(css.biomesScreen)}>
        
        <h3 className={classes(css.biomesScreenTitle)}>
            <img className={classes(css.biomesScreenIcon)} src='./images/biomes.svg' />
            Biomes
        </h3>
        ...
        <div style={{display: 'flex'}}>
            ...
            {
                (
                    <div className={classes(css.biomeData, !currentBiomeData && css.biomeDataHidden)}>
                        <div className={css.biomeName}>
                            {currentBiomeData?.name ?? 'Select a Biome'}
                        </div>
                        {currentBiomeData ? <div className={css.biomeRankWrapper}>
                            <div className={css.biomeRank}> 
                                Rank: {currentBiomeData?.biomeRank}
                            </div>
                            <div className={css.biomeRank}>
                                Prevalance: {currentBiomeData?.sqKm}/{'2555'}{'km'}
                            </div>
                        </div> : null}
                        <div className={css.biomeRankItems}>
                            {currentBiomeData?.rankBonuses && currentBiomeData?.rankBonuses.map((rankBonus: any, idx: number) => (
                                <div style={{
                                    marginRight: idx * 2 + 'px',
                                }} key={rankBonus.bonus} className={css.biomeRankItem}>
                                    {rankBonus?.rank} {rankBonus.unlocked ? addImages(rankBonus.bonus) : '???'}
                                </div>
                            ))}
                        </div>
                    </div>
