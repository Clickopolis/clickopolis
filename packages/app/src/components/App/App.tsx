...
export class AppBase extends React.Component<AppProps> {
  ...
  componentDidMount() {
    ...
    this.intervalIdDecisecond = requestInterval(
      this.decisecondTimer,
      1000 / 10
    );
  ...
  decisecondTimer = () => {
    const { food, production, createProduction, growFood, flags, timeStatus } =
      this.props;
  ...
  minuteTimer = () => {
    const { updateCivilization, civilization, gainCash, flags, timeStatus } =
      this.props;
  ...
  public render() {
    ...
    return (
      <>
        ...
        <div
          id="app"
          className={`clickopolis-app ${
            timeStatus === TimeStatus.Paused && 'paused'
          }`}
        >
