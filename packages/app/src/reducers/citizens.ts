...
export function merchant(state = defaults.get('merchant'), action: Action<ADD_CITIZEN | REMOVE_CITIZEN>) {
    const amount = parseInt(action.amount);
    switch (action.type) {
        case ADD_CITIZEN:
            return action.citizen === 'merchant' ? { ...state, amount: state.amount + amount } : state;
        case REMOVE_CITIZEN:
            return action.citizen === 'merchant' ? { ...state, amount: Math.max(state.amount + (-amount), 0) } : state;
        default:
            return state;
    }
}
...
export function doctor(state = defaults.get('doctor'), action: Action<ADD_CITIZEN | REMOVE_CITIZEN>) {
    const amount = parseInt(action.amount);
    switch (action.type) {
        case ADD_CITIZEN:
            return action.citizen === 'doctor' ? { ...state, amount: state.amount + amount } : state;
        case REMOVE_CITIZEN:
            return action.citizen === 'doctor' ? { ...state, amount: Math.max(state.amount + (-amount), 0) } : state;
        default:
            return state;
    }
}
